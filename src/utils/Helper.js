...
export const showPrice = (price) => {
  ...
  let settings;
  try {
    settings = store.getState()?.settingsData?.settings?.app_settings;
  } catch (error) {
    settings = null;
  }
  const currencyCode = settings?.country_currency_code || "USD"; // Currency code for locale formatting rules
  const currencySymbol = settings?.currency || "$"; // Custom currency symbol to display
