...
const LoginModal = ({
  ...
  const ensureFcmToken = useCallback(async () => {
    ...
    ensureFcmPromiseRef.current = new Promise(async (resolve) => {
      // Proactive check: If notifications are denied, don't even try to fetch.
      // This prevents the promise from hanging if fetchToken doesn't handle 'denied' well.
      ...
      try {
        await fetchToken(
          // Error callback - notifications denied or not supported
          () => {
            resolve(null);
          },
          (token) => {
